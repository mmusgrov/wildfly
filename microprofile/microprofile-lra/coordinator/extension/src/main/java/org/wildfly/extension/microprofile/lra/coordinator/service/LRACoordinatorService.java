...
public final class LRACoordinatorService implements Service {
    ...
    private static final String longRunningActionType = LongRunningAction.getType().substring(1);
    private static final String failedLongRunningActionType = FailedLongRunningAction.getType().substring(1);

    private final Supplier<Host> undertow;
    private final Supplier<ObjStoreBrowser> objStoreBrowserSupplier;

    private volatile Deployment deployment = null;

    public LRACoordinatorService(final Supplier<Host> undertow, final Supplier<ObjStoreBrowser> objStoreBrowserSupplier) {
        this.undertow = undertow;
        this.objStoreBrowserSupplier = objStoreBrowserSupplier;
    }

    @Override
    public synchronized void start(final StartContext context) throws StartException {
        deployCoordinator();
        addTypesToObjStoreBrowser();
    }
    ...
    private void addTypesToObjStoreBrowser() {
        ObjStoreBrowser objStoreBrowser = objStoreBrowserSupplier.get();
        objStoreBrowser.addType(longRunningActionType, LongRunningAction.class.getName(), LRAActionBean.class.getName());
        objStoreBrowser.addType(failedLongRunningActionType, FailedLongRunningAction.class.getName(), LRAActionBean.class.getName());
    }
